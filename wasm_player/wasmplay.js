...
var createWASMplay = (() => {
  ...
  return (
async function(createWASMplay = {})  {

var Module = typeof createWASMplay != "undefined" ? createWASMplay : {};
...
if (Module["locateFile"]) {
 wasmBinaryFile = "wasmplay.wasm";
 if (!isDataURI(wasmBinaryFile)) {
  wasmBinaryFile = locateFile(wasmBinaryFile);
 }
} else {
 wasmBinaryFile = new URL("wasmplay.wasm", import.meta.url).href;
}
...
var ASM_CONSTS = {
 5170288: $0 => {
  try {
   let _rinfo = Emval.toValue($0);
   console.log("playerRender EM_ASM " + JSON.stringify(_rinfo, null, "\t"));
   let _jsInfo = {
    "videoStreamID": _rinfo["videoStreamID"],
    "subtitleStreamID": _rinfo["subtitleStreamID"],
    "audioStreamID": _rinfo["audioStreamID"],
    "videoData": [],
    "videoLineSize": _rinfo["videoLineSize"],
    "videoTimer": _rinfo["videoTimer"],
    "videoDuration": _rinfo["videoDuration"],
    "videoWidth": _rinfo["videoWidth"],
    "videoHeight": _rinfo["videoHeight"],
    "videoPixelFormat": _rinfo["videoPixelFormat"],
    "subtitleStartTime": _rinfo["subtitleStartTime"],
    "subtitleEndTime": _rinfo["subtitleEndTime"],
    "subtitleData": [],
    "subtitleType": _rinfo["subtitleType"],
    "audioData": [],
    "audioSampleRate": _rinfo["audioSampleRate"],
    "audioSampleFormat": _rinfo["audioSampleFormat"],
    "audioSampleNumber": _rinfo["audioSampleNumber"],
    "audioDuration": _rinfo["audioDuration"],
    "audioStartTime": _rinfo["audioStartTime"],
    "audioChannelLayout": _rinfo["audioChannelLayout"],
    "audioChannels": _rinfo["audioChannels"],
    "audioFrameSize": _rinfo["audioFrameSize"],
    "audioBytesPerSec": _rinfo["audioBytesPerSec"]
   };
   console.log("BX: playerRender jsinfo " + JSON.stringify(_jsInfo, null, "\t"));
   for (let idx = 0; idx < _rinfo.videoData.length; idx++) {
    let dataPtr = _rinfo.videoData[idx];
    let dataSize = _rinfo.videoDataSize[idx];
    _jsInfo.videoData.push({
     "data": HEAPU8.subarray(dataPtr, dataPtr + dataSize)
    });
   }
   if (_rinfo.audioSampleNumber > 0) {
    for (let idx = 0; idx < _rinfo.audioData.length; idx++) {
     let ad = _rinfo.audioData[idx];
     if (ad % 4) {
      throw "BX: playerRender float pointer must alained with 4";
     }
     console.log(`BX: make audio float array [${ad >> 2}, ${(ad >> 2) + _rinfo.audioSampleNumber}] `);
     let farray = HEAPF32.subarray(ad >> 2, (ad >> 2) + _rinfo.audioSampleNumber);
     _jsInfo.audioData.push(farray);
    }
   }
   if (_rinfo.subtitleType == 1) {
    for (let idx = 0; idx < _rinfo.subtitleData.length; idx++) {
     console.log(`BX: make subtitle js array`);
     let h = _rinfo.subtitleData[idx].height;
     let size = _rinfo.subtitleData[idx].linesize;
     let nb_colors = _rinfo.subtitleData[idx].nb_colors;
     let pixelPtr = _rinfo.subtitleData[idx].pixels;
     let palettePtr = _rinfo.subtitleData[idx].palette;
     _jsInfo.subtitleData.push({
      "x": _rinfo.subtitleData[idx].x,
      "y": _rinfo.subtitleData[idx].y,
      "width": _rinfo.subtitleData[idx].width,
      "height": h,
      "nb_colors": nb_colors,
      "linesize": size,
      "pixels": HEAPU8.subarray(pixelPtr, pixelPtr + h * size),
      "palette": HEAPU8.subarray(palettePtr, palettePtr + nb_colors * 4)
     });
    }
   } else {
    _jsInfo.subtitleData = _rinfo["subtitleData"];
   }
   window.render_to_canvas(_jsInfo);
  } catch (err) {
   console.error("render JS exception: " + err + " stack:" + err.stack);
  }
 },
 5172955: $0 => {
  let md = Emval.toValue($0);
  window.emitWASMPlayerMetadataEvent(md);
  console.log("emitted stream " + md.id + " type ", md.type);
 },
 5173084: ($0, $1) => {
  let evtType = Emval.toValue($0);
  let evtPriv = Emval.toValue($1);
  window.emitWASMPlayerEvent(evtType, evtPriv);
  console.log("emitted player event type ", evtType, evtPriv);
 },
 5173261: $0 => {
  let fileList = Emval.toValue($0);
  Asyncify.handleAsync(async () => {
   let _om = self.onmessage;
   console.log("player_inf: workerMountFile EM_ASM ", fileList);
   let rspWaiter = new Promise((resolve, reject) => {
    self.onmessage = event => {
     console.log("hook: worker received ", event.data);
     try {
      if (event.data.cmd == "_rsp_get_file") {
       console.log("workerMountFile: _rsp_get_file");
       let data = event.data;
       console.log(`workerMountFile: get ${data.files.length} file from main `);
       for (let file of data.files) {
        console.log("workerMountFile: mounting file ", file.name);
       }
       if (!Module.FS.analyzePath("/workerfs").exists) {
        Module.FS.mkdir("/workerfs");
       }
       let mountpoint;
       let mountIndex = 0;
       let maxTries = 10;
       for (mountIndex = 0; mountIndex < maxTries; ++mountIndex) {
        mountpoint = "/workerfs_impl_" + mountIndex;
        if (!Module.FS.analyzePath(mountpoint).exists) {
         Module.FS.mkdir(mountpoint);
        } else {
         continue;
        }
        console.log(`workerMountFile: mountpoint is ${mountpoint}`);
        Module.FS.mount(WORKERFS, {
         files: data.files
        }, mountpoint);
        for (let file of data.files) {
         let mountedFile = mountpoint + "/" + file.name;
         console.log(`workerMountFile: mounted file state ${file.name} `, Module.FS.stat(mountedFile));
         Module.FS.symlink(mountedFile, "/workerfs/" + file.name);
        }
        break;
       }
       if (mountIndex == maxTries) {
        console.log(`workerMountFile: reached max tries when get new mountpoint`);
        throw new Error("workerMountFile: reached max tries when get new mountpoint");
       }
       for (let file of data.files) {
        console.log(`workerMountFile: mounted file state ${file.name} `, Module.FS.stat("/workerfs/" + file.name));
       }
       resolve(1);
       return;
      }
   ...
   let result = await rspWaiter;
   console.log("hack: wokerMountFS get result ", result);
  });
 },
 5175283: $0 => {
  let filename = Emval.toValue($0);
  let result = Module.FS.analyzePath(filename);
  console.log("workerCheckFSPath: analyzePath ", result);
 }
};
...
var PThread = {
 ...
 allocateUnusedWorker: function() {
  var worker;
  if (!Module["locateFile"]) {
   worker = new Worker(new URL("wasmplay.worker.js", import.meta.url));
  } else {
   var pthreadMainJs = locateFile("wasmplay.worker.js");
...
function ___syscall_faccessat(dirfd, path, amode, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(7, 1, dirfd, path, amode, flags);
 try {
...
function ___syscall_fcntl64(fd, cmd, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(8, 1, fd, cmd, varargs);
...
function ___syscall_fstat64(fd, buf) {
 if (ENVIRONMENT_IS_PTHREAD) {
  ...
  let pathStr = stream.path;
  let dirStr = PATH.dirname(pathStr);
  if (!dirStr.startsWith("/workerfs")) return proxyToMainThread(9, 1, fd, buf);
 }
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function ___syscall_getsockopt(fd, level, optname, optval, optlen, d1) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 1, fd, level, optname, optval, optlen, d1);
 try {
  var sock = getSocketFromFD(fd);
  if (level === 1) {
   if (optname === 4) {
...
function ___syscall_ioctl(fd, op, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 1, fd, op, varargs);
 SYSCALLS.varargs = varargs;
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  switch (op) {
...
function ___syscall_listen(fd, backlog) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(15, 1, fd, backlog);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_lstat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) {
  let pathStr = SYSCALLS.getStr(path);
  let dirStr = PATH.dirname(pathStr);
  if (!dirStr.startsWith("/workerfs")) return proxyToMainThread(16, 1, path, buf);
 }
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_mkdirat(dirfd, path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(17, 1, dirfd, path, mode);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  path = PATH.normalize(path);
...
function ___syscall_newfstatat(dirfd, path, buf, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(18, 1, dirfd, path, buf, flags);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_openat(dirfd, path, flags, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) {
  let pathStr = SYSCALLS.getStr(path);
  let dirStr = PATH.dirname(pathStr);
  if (!dirStr.startsWith("/workerfs")) return proxyToMainThread(19, 1, dirfd, path, flags, varargs);
 }
 SYSCALLS.varargs = varargs;
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_rmdir(path) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(23, 1, path);
 try {
  path = SYSCALLS.getStr(path);
  FS.rmdir(path);
  return 0;
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(24, 1, fd, message, length, flags, addr, addr_len);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_socket(domain, type, protocol) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(25, 1, domain, type, protocol);
 try {
...
function ___syscall_stat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) {
  let pathStr = SYSCALLS.getStr(path);
  let dirStr = PATH.dirname(pathStr);
  if (!dirStr.startsWith("/workerfs")) return proxyToMainThread(26, 1, path, buf);
 }
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_unlinkat(dirfd, path, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(27, 1, dirfd, path, flags);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  if (flags === 0) {
...
function __mmap_js(len, prot, flags, fd, off, allocated, addr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 1, len, prot, flags, fd, off, allocated, addr);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 1, addr, len, prot, flags, fd, offset);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (prot & 2) {
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) {
  ...
  let pathStr = stream.path;
  let dirStr = PATH.dirname(pathStr);
  if (!dirStr.startsWith("/workerfs")) return proxyToMainThread(32, 1, fd);
 }
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
...
function _fd_fdstat_get(fd, pbuf) {
 if (ENVIRONMENT_IS_PTHREAD) {
  ...
  let pathStr = stream.path;
  let dirStr = PATH.dirname(pathStr);
  if (!dirStr.startsWith("/workerfs")) return proxyToMainThread(33, 1, fd, pbuf);
 }
 try {
  var rightsBase = 0;
...
function _fd_read(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) {
  ...
  let pathStr = stream.path;
  let dirStr = PATH.dirname(pathStr);
  if (!dirStr.startsWith("/workerfs")) return proxyToMainThread(34, 1, fd, iov, iovcnt, pnum);
 }
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) {
  ...
  let pathStr = stream.path;
  let dirStr = PATH.dirname(pathStr);
  if (!dirStr.startsWith("/workerfs")) return proxyToMainThread(35, 1, fd, offset, whence, newOffset);
 }
 try {
...
function _getaddrinfo(node, service, hint, out) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(37, 1, node, service, hint, out);
 var addr = 0;
 var port = 0;
 var flags = 0;
 var family = 0;
...
var wasmImports = {
 ...
 "Na": ___syscall_poll,
 "Ma": ___syscall_recvfrom,
 "La": ___syscall_renameat,
 "Ka": ___syscall_rmdir,
 ...
 "X": _emscripten_cancel_main_loop,
 ...
 "ka": _emscripten_num_logical_cores,
 "ja": _emscripten_receive_on_main_thread_js,
 "ia": _emscripten_resize_heap,
 "ha": _emscripten_set_main_loop_arg,
...
var _main = Module["_main"] = function() {
 return (_main = Module["_main"] = Module["asm"]["gb"]).apply(null, arguments);
};
...
var __emscripten_tls_init = Module["__emscripten_tls_init"] = function() {
 return (__emscripten_tls_init = Module["__emscripten_tls_init"] = Module["asm"]["nb"]).apply(null, arguments);
};
...
var __emscripten_thread_crashed = Module["__emscripten_thread_crashed"] = function() {
 return (__emscripten_thread_crashed = Module["__emscripten_thread_crashed"] = Module["asm"]["tb"]).apply(null, arguments);
};
...
var __emscripten_run_in_main_runtime_thread_js = function() {
 return (__emscripten_run_in_main_runtime_thread_js = Module["asm"]["vb"]).apply(null, arguments);
};
...
var __emscripten_thread_free_data = function() {
 return (__emscripten_thread_free_data = Module["asm"]["wb"]).apply(null, arguments);
};

var __emscripten_thread_exit = Module["__emscripten_thread_exit"] = function() {
 return (__emscripten_thread_exit = Module["__emscripten_thread_exit"] = Module["asm"]["xb"]).apply(null, arguments);
};

var __emscripten_check_mailbox = Module["__emscripten_check_mailbox"] = function() {
 return (__emscripten_check_mailbox = Module["__emscripten_check_mailbox"] = Module["asm"]["yb"]).apply(null, arguments);
};
...
var _emscripten_stack_set_limits = function() {
 return (_emscripten_stack_set_limits = Module["asm"]["Ab"]).apply(null, arguments);
};
...
var dynCall_iiiii = Module["dynCall_iiiii"] = function() {
 return (dynCall_iiiii = Module["dynCall_iiiii"] = Module["asm"]["Lb"]).apply(null, arguments);
};

var dynCall_iij = Module["dynCall_iij"] = function() {
 return (dynCall_iij = Module["dynCall_iij"] = Module["asm"]["Mb"]).apply(null, arguments);
};
...
var dynCall_viiiiii = Module["dynCall_viiiiii"] = function() {
 return (dynCall_viiiiii = Module["dynCall_viiiiii"] = Module["asm"]["Ob"]).apply(null, arguments);
};
...
var dynCall_viiiiiiii = Module["dynCall_viiiiiiii"] = function() {
 return (dynCall_viiiiiiii = Module["dynCall_viiiiiiii"] = Module["asm"]["Rb"]).apply(null, arguments);
};
...
var dynCall_iiiijj = Module["dynCall_iiiijj"] = function() {
 return (dynCall_iiiijj = Module["dynCall_iiiijj"] = Module["asm"]["Ub"]).apply(null, arguments);
};

var _asyncify_start_unwind = function() {
 return (_asyncify_start_unwind = Module["asm"]["Vb"]).apply(null, arguments);
};

var _asyncify_stop_unwind = function() {
 return (_asyncify_stop_unwind = Module["asm"]["Wb"]).apply(null, arguments);
};

var _asyncify_start_rewind = function() {
 return (_asyncify_start_rewind = Module["asm"]["Xb"]).apply(null, arguments);
};

var _asyncify_stop_rewind = function() {
 return (_asyncify_stop_rewind = Module["asm"]["Yb"]).apply(null, arguments);
};

var _ff_h264_cabac_tables = Module["_ff_h264_cabac_tables"] = 1926764;
...
run();
  ...
  return createWASMplay.ready
}

);
})();
export default createWASMplay;