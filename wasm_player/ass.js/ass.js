  ...
  for (var i = 1; i < lines.length; i++) {
    ...
    if (section == "events") {
      ...
      if (/^Format:/i.test(lines[i])) {
        ...
        if(eventsFormat[eventsFormat.length-1] != 'Text') {
          throw new Error("ASS event formats last field must be 'Text' ");
        }
      }
...
// testStyle = {};
Ass.prototype.applyStylesToDialogue = function(dialogue, container, scale) {
    ...
    switch (dialogue.taggedText[j].type) {
      case "text":
        if (drawMode == 0) {
          ...
          modifiedStyle.toCss(span.style, null, scale);
          ...
          console.log('text draw mode '+drawMode);
        }
        break;
      case "p":
      ...
      default:
        ...
        console.log('BX: tagged type '+dialogue.taggedText[j].type);
...
// Refactory from origin code, there is no return value. It's maybe incomplete code.
Ass.prototype.applyStyles = function() {
  for (var i = 0; i < ass.dialogue.length; i++) {
    var dialogue = this.dialogue[i];
    this.applyStylesToDialogue(dialogue);
  }
}

Ass.prototype.parseEvent = function(eventLine, eventsFormat) {
  if(!eventsFormat) {
    eventsFormat = this.eventsFormat;
  }
  var tmpEvent = new Object();
  var content = eventLine.replace(/^Dialogue: */i, "");
  let parsePos = 0;
  for (let idx = 0; idx < eventsFormat.length - 1; idx ++) { // Don't parse 'Text' field
    let nextPos = content.indexOf(',', parsePos);
    if(nextPos == -1) { // reach end position
      tmpEvent[eventsFormat[idx]] = content.slice(parsePos);
      parsePos = nextPos;
      break;
    } else {
      tmpEvent[eventsFormat[idx]] = content.slice(parsePos, nextPos);
      parsePos = nextPos + 1;
    }
  }
  if(parsePos < 0) {
    ...
    tmpEvent['Text'] = content.slice(parsePos);
  }
  
  this.lineNumber += 1;
  tmpEvent.LineNumber = this.lineNumber;
  return tmpEvent;
}
...
Ass.Dialogue.OverrideTag.prototype.applyTo = function(style) {
  ...
  switch (this.type) {
    ...
    case "2c":
      ...
      break;
    case "3c":
      console.log('BX: 3c value '+JSON.stringify(args, null, '\t'));
      if (args.length == 1) {
...
Ass.AssStyle.prototype.toCss = function(css, time, scale) {
  ...
  css.fontSize = px(this.fontSize.get(time)*scale);
  // css.fontSize = px(36);
  ...
  console.log('BX: toCss border style '+this.borderStyle);
  console.log('BX: toCss border style '+this.borderStyle);
  if(this.baseStyle.borderStyle == 1) { 
    css.textShadow = `${this.shadowX.value}px ${this.shadowY.value}px ${this.blur}px ${rgba(this.shadowColorR.value, this.shadowColorG.value, this.shadowColorB.value, this.shadowColorA.value)}`;
    css.webkitTextStroke = `${this.borderStyle}px transparent`;
    // css.webkitTextStroke = `1px transparent`;
    css.background = `${rgba(this.outlineColorR.value, this.outlineColorG.value, this.outlineColorB.value, this.outlineColorA.value)}`;
...
Ass.AssStyle.fromRaw = function(raw) {
  ...
  if (parseInt(raw.BorderStyle) == 1) {
    style.borderStyle = 1;
  } else if(parseInt(raw.BorderStyle) == 3) {
    style.borderStyle = 3;
  }
